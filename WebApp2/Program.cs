...
// FIS proxy via Kong with user's access token
async Task<IResult> CallFis(HttpContext ctx, string lobPath, IHttpClientFactory f)
{
    var accessToken = ctx.Session.GetString("token.access_token");
    if (string.IsNullOrEmpty(accessToken))
    {
        Console.WriteLine($"[AUTH] FIS call '{lobPath}' rejected. Access token missing from session.");
        return Results.Unauthorized();
    }
...
app.MapGet("/api/call-lob1", (HttpContext ctx, IHttpClientFactory f) => CallFis(ctx, "lob1", f));
app.MapGet("/api/call-lob2", (HttpContext ctx, IHttpClientFactory f) => CallFis(ctx, "lob2", f));
app.MapGet("/api/call-lob3", (HttpContext ctx, IHttpClientFactory f) => CallFis(ctx, "lob3", f));
